...
ifeq ($(shell uname),Darwin)
	...
	LINKSHARED += -shared -Wl,-soname,libtree-sitter-$(PARSER_NAME).so.$(SONAME_MAJOR)
endif
...
bindings/c/tree-sitter-$(PARSER_NAME).pc:
	sed -e 's|@LIBDIR@|$(LIBDIR)|;s|@INCLUDEDIR@|$(INCLUDEDIR)|;s|@VERSION@|$(VERSION)|' \
		...
		bindings/c/tree-sitter-$(PARSER_NAME).pc.in > $@
...
clean:
	...
	rm -f bindings/c/tree-sitter-$(PARSER_NAME).pc
